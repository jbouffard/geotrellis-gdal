...
object NetCdfInputFormat {
  ...
  /** This method reads the type and date from a NetCdf time units string.
    ...
    * @param dateTimeFormat The format for the date time format. This String needs
    *                       to specify where the type is, and where the year is declared.
    *                       It can also declare the months and days. For our example string
    *                       above the DateTimeFormat should look be "TTTT*******YYYY*MM*DD".
    *                       You can also use hours (h), minutes (m) or seconds 's'.
    *
  ...
  def readTypeAndDate(
    ...
    yearOffset: Int = 0,
    monthOffset: Int = 0,
    dayOffset: Int = 0): (String, ZonedDateTime) = {
  ...
  /**
    ...
    */
  def incrementDate(timeType: String, v: Double, base: ZonedDateTime): ZonedDateTime =
    timeType match {
