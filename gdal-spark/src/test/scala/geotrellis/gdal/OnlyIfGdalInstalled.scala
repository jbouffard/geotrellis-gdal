...
trait OnlyIfGdalInstalled extends FunSpec with BeforeAndAfterAll {
  ...
  val jpeg2000Drivers: List[String] = List("JPEG2000", "JP2ECW", "JP2KAK", "JP2Lura", "JP2MrSID", "JP2OpenJPEG", "JPIPKAK")

  // Only run f if some GDAL JPEG2000 plugin is installed.
  def ifGdalWithJpeg2000Installed(f: => Unit): Unit = ifGdalInstalled {
